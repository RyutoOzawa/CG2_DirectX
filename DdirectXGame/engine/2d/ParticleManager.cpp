...
void ParticleManager::StaticInitialize(ReDirectX* directX)
{
	// nullptrチェック
	assert(directX);

	ParticleManager::device = directX->GetDevice();
	ParticleManager::cmdList = directX->GetCommandList();
...
void ParticleManager::Draw(Camera camera)
{
	...
	srvGpuHandle.ptr += incrementSize * textureHandle_;
