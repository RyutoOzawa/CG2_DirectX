...
void PostEffect::BeginDrawScene(ID3D12GraphicsCommandList* cmdList)
{
	...
	for (int i = 0; i < 2; i++) {
		viewports[i].Width = WindowsAPI::winW;
		viewports[i].Height = WindowsAPI::winH;
		viewports[i].TopLeftX = 0;
		viewports[i].TopLeftY = 0;
		viewports[i].MinDepth = 0.0f;
		viewports[i].MaxDepth = 1.0f;
		scissorRects[i].left = 0; // 切り抜き座標左
		scissorRects[i].right = scissorRects[i].left + WindowsAPI::winW; // 切り抜き座標右
		scissorRects[i].top = 0; // 切り抜き座標上
		scissorRects[i].bottom = scissorRects[i].top + WindowsAPI::winH; // 切り抜き座標下
	}
...
void PostEffect::CreatePipeline2D()
{
	...
	//CD3DX12_DESCRIPTOR_RANGE test;
	//test.Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV,)

	//デスクリプタレンジの設定
	D3D12_DESCRIPTOR_RANGE descriptorRange0{};
	descriptorRange0.NumDescriptors = 1;		//一度の描画に使うテクスチャが1枚なので1
	...
	descriptorRange1.NumDescriptors = 1;		//一度の描画に使うテクスチャが1枚なので1
	descriptorRange1.RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_SRV;
	descriptorRange1.BaseShaderRegister = 1;	//テクスチャレジスタ1番
	...
	rootParams[2].DescriptorTable.pDescriptorRanges = &descriptorRange1;		//デスクリプタレンジ
