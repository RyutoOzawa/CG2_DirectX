...
PipelineSet CreatePipeline2D(ID3D12Device* dev);
...
#pragma region パイプラインとルートシグネチャの生成
...
#pragma endregion
	...
	//3Dオブジェクト用パイプライン生成
	PipelineSet object3DPipelineSet = CreatepipeLine3D(directX->GetDevice());
	//スプライト用パイプライン生成
	PipelineSet SpritePipelineSet = CreatePipeline2D(directX->GetDevice());
...
#pragma region DirectX毎フレーム処理
		...
		directX->GetCommandList()->SetPipelineState(object3DPipelineSet.pipelineState.Get());
		directX->GetCommandList()->SetGraphicsRootSignature(object3DPipelineSet.rootSignature.Get());
